...
class deepsem_generation:
    ...
    def train_model(self):
        ...
        opt = self.opt
        vae = VAE_EAD(adj_A_init, 1, opt.n_hidden, opt.K).float().cpu()
        ...
        for epoch in range(opt.n_epochs):
            ...
            for i, data_batch in enumerate(dataloader, 0):
                ...
                loss, loss_rec, loss_gauss, loss_cat, dec, y, hidden = vae(inputs, dropout_mask=dropout_mask.cpu(),
        ...
        for data_batch in dataloader:
            ...
            inputs, data_id, dropout_mask = data_batch
            inputs = Variable(inputs.type(Tensor))
            loss, loss_rec, loss_gauss, loss_cat, dec, y, hidden = vae(inputs, dropout_mask=dropout_mask.cpu(), opt=opt)
