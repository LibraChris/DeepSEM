...
class LossFunctions:
    ...
    def log_normal(self, x, mu, var):
        ...
        return -0.5 * torch.mean(
            torch.log(torch.FloatTensor([2.0 * np.pi]).cpu()).sum(0) + torch.log(var) + torch.pow(x - mu, 2) / var, dim=-1)
...
class VAE_EAD(nn.Module):
    ...
    def forward(self, x, dropout_mask, temperature=1.0, opt=None, ):
        ...
        mask = Variable(torch.from_numpy(np.ones(self.n_gene) - np.eye(self.n_gene)).float(), requires_grad=False).cpu()
